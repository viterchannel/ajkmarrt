...
async function sendReport(report: Record<string, unknown>): Promise<void> {
  try {
    ...
    /* Use admin session token instead of HMAC secret.
       The secret was previously bundled in the client JS via VITE_
       env var, which is a security risk.  The server validates the
       admin session token on the receiving endpoint. */
