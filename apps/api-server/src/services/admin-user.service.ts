...
export class UserService {
  ...
  static async createAdminAccount(input: AdminAccountInput) {
    ...
    const emailValue = input.email ? input.email.trim().toLowerCase() || null : null;
  ...
  static async getOtpAuditLog(filters?: {
    ...
    to?: string;
    page?: number;
    category?: "all" | "bypass" | "fail" | "admin" | "activity";
  }) {
    ...
    const ACTIVITY_EVENTS = ["otp_sent", "otp_verified", "otp_verified_new_user"];
    const ALL_OTP_EVENTS = [
      ...ACTIVITY_EVENTS,
      ...BYPASS_EVENTS,
      ...FAIL_EVENTS,
      ...ADMIN_EVENTS,
    ...
    let otpEvents: string[];
    switch (filters?.category) {
      case "bypass":   otpEvents = BYPASS_EVENTS;   break;
      case "fail":     otpEvents = FAIL_EVENTS;      break;
      case "admin":    otpEvents = ADMIN_EVENTS;     break;
      case "activity": otpEvents = ACTIVITY_EVENTS;  break;
      default:         otpEvents = ALL_OTP_EVENTS;   break;
    }
    ...
    if (filters?.from)
      conditions.push(sql`${authAuditLogTable.createdAt} >= ${new Date(filters.from)}`);
    if (filters?.to)
      conditions.push(sql`${authAuditLogTable.createdAt} <= ${new Date(filters.to)}`);
    ...
    const enriched = rows.map((row) => {
      ...
      const derivedResult = metaResult ?? (FAIL_EVENT_SET.has(row.event) ? "fail" : "success");
  ...
  /**
   * Generate OTP for a user looked up by phone or email identifier.
   * Used by the admin "View Current OTP" tool.
   */
  static async generateOtpByIdentifier(identifier: string) {
    const trimmed = identifier.trim();

    const canonical = canonicalizePhone(trimmed);
    ...
    if (canonical) {
      const rows = await db
        .select({ id: usersTable.id, phone: usersTable.phone, name: usersTable.name, email: usersTable.email })
        .from(usersTable)
        .where(eq(usersTable.phone, canonical))
        .limit(1);
      foundUser = rows[0];
    }

    if (!foundUser && trimmed.includes("@")) {
      const rows = await db
        .select({ id: usersTable.id, phone: usersTable.phone, name: usersTable.name, email: usersTable.email })
        .from(usersTable)
        .where(eq(usersTable.email, trimmed.toLowerCase()))
        .limit(1);
      foundUser = rows[0];
    }
    ...
    const result = await UserService.generateOtpForUser(foundUser.id);
    return { ...result, userId: foundUser.id, name: foundUser.name, email: foundUser.email ?? null };
  }
