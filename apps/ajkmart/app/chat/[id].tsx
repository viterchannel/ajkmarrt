...
const { colors: C } = useTheme();
  ...
  const chatCacheKey = `comm_chat_messages:${id}`;

  const persistChatMessages = useCallback(async (msgs: Message[]) => {
    try {
      await AsyncStorage.setItem(chatCacheKey, JSON.stringify(msgs));
    } catch (err) {
      log.warn("Failed to persist chat messages:", err instanceof Error ? err.message : String(err));
    }
  }, [chatCacheKey]);
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      /* (a) Load cached messages immediately so history is visible on cold launch */
      try {
        ...
        log.warn("Failed to load cached messages:", err instanceof Error ? err.message : String(err));
      }

      /* (b) Fetch last 50 messages from backend and merge (deduplicate by id) */
      try {
        const msgs: Message[] = await apiFetch(`/conversations/${id}/messages?limit=50`);
        setMessages(prev => {
          ...
          /* (c) Persist merged result back to AsyncStorage */
          persistChatMessages(merged).catch(() => {});
          return merged;
        });
    ...
    socket.on("comm:message:new", (msg: Message) => {
      setMessages(prev => {
        ...
        const next = [...prev, msg];
        /* Persist updated list so history survives app restart */
        persistChatMessages(next).catch(() => {});
        return next;
      });
    });
