...
function StarDistributionBar({
  ...
  return (
    <div className="space-y-2 rounded-2xl border border-white/[0.06] bg-card-dark p-3.5 backdrop-blur-sm">
      {[5, 4, 3, 2, 1].map((star) => {
...
function SkeletonCard() {
  return (
    <div className="animate-pulse space-y-3 rounded-3xl border border-white/10 bg-card-dark p-4">
      <div className="flex items-center justify-between">
        <div className="h-3.5 w-24 rounded-full bg-border-dark" />
        <div className="h-3 w-16 rounded-full bg-border-dark" />
      </div>
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="h-3.5 w-3.5 rounded-full bg-border-dark" />
        ))}
      </div>
      <div className="h-3 w-3/4 rounded-full bg-border-dark" />
    </div>
  );
}
    ...
    <div className="min-h-screen bg-page-bg">
      ...
      >
        ...
        <div className="relative mb-5 flex items-center gap-3">
          <Link
            href="/profile"
            className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/[0.07] bg-card-dark/[0.07] text-white/70 transition-colors hover:bg-card-dark/[0.12]"
          >
            <ArrowLeft size={18} />
          </Link>
          <div>
            ...
            <h1 className="text-2xl font-extrabold tracking-tight text-white">{T("myReviews")}</h1>
          </div>
        </div>
      ...
      <div className="space-y-3 px-4 pt-4 pb-8">
        {isLoading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-base font-bold text-[#B0B0B0]">{T("noReviewsYet")}</p>
          ...
          reviews.map((review) => (
            <div
              key={review.id}
              className="space-y-2.5 rounded-3xl border border-white/10 bg-card-dark p-4 shadow-sm"
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex-shrink-0 text-right">
                  ...
                  {(review.orderId || review.rideId) && (
                    <p className="mt-1 max-w-[120px] truncate font-mono text-[10px] text-[#B0B0B0]">
                      #{(review.orderId ?? review.rideId ?? "").slice(-8).toUpperCase()}
                    </p>
                  )}
                  <p className="mt-0.5 text-[10px] text-[#B0B0B0]">{formatDate(review.createdAt)}</p>
                </div>
              </div>

              {review.comment && review.comment.trim() && (
                <div className="rounded-2xl bg-card-dark px-3.5 py-2.5">
                  <p className="text-sm leading-relaxed text-[#B0B0B0] italic">
                    "{review.comment.trim()}"
                  </p>
                </div>
              )}
              ...
              <div className="flex items-center gap-1.5">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    className={`h-1 flex-1 rounded-full ${i <= Math.floor(review.rating) ? "bg-warning" : "bg-border-dark"}`}
                  />
                ))}
              </div>
            </div>
