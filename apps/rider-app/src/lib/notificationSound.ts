...
/**
 * Resume the AudioContext whenever the page becomes visible again.
 * Browsers suspend AudioContext when the tab/app goes to background.
 * Registering this listener once ensures sounds work on return without
 * the rider needing to tap the "unlock audio" banner again.
 */
export function initAudioContextRevival(): () => void {
  const handler = () => {
